...
def prepare_input_w_flow(path, num_frames,gray=False):
    ...
    if not gray:
        input_flow_forward,input_flow_backward = utils.read_flow_sequence(path.replace("pics","flo"), num_frames=num_frame)
...
if is_training:
    for epoch in range(1,maxepoch):
        ...
        # assert 2==3
        for id in np.random.permutation(len(os.listdir(imgs_dir))):#1431167
            st=time.time()
            color_image = np.float32(imageio.imread(os.path.join(imgs_dir, os.listdir(imgs_dir)[id])))/255.0
            # color_image=np.float32(scipy.misc.imread(os.path.join(imgs_dir, os.listdir(imgs_dir)[id])))/255.0
...
# Inference
    ...
    for ind in range(numtest):
        ...
        input_image_src, input_image_target, input_flow_forward_src, input_flow_backward_src = prepare_input_w_flow(test_low[ind],num_frames=num_frame,gray=False)
        if input_image_src is None or input_image_target is None or input_flow_forward_src is None:
            print("Not able to read the images/flows.")
            continue
        st=time.time()
